...
class FiberScalar:
    """ Scalar information pertaining to a single fiber """
    
    def __init__(self):
        self.tree_scalar = tree()
        
    def getReverseScalar(self, fiberArray):
        """ Determine the scalar quantity corresponding to reverse order of 
        the fiber.
        ...
        fiberScalar = FiberScalar()
        
        fiberScalar.pts_per_fiber = fiberArray.pts_per_fiber

        for pidx in range(0, fiberArray.pts_per_fiber):
            fiberScalar.tree_scalar[0][pidx] = \
                self.tree_scalar[0][fiberArray.pts_per_fiber - (pidx+1)]
...
class FiberArrayScalar:
    ...
    def getScalar(self, fiberArray, scalarType):
        ...
        OUTPUT:
            scalarList - List of scalar vlaues indexed by fiber and point 
        """
